...
describe("testing the help functions", () => {
  ...
  describe("testing convertDate", () => {
    it("normal case", (done) => {
      const result = AwesomeHelp.convertDate(
        new Date(2019, 7, 27, 9, 0, 0),
        "YYYY-MM-DD hh:mm:ss"
      );
  ...
  describe("testing the checkSensitiveWord function", () => {
    it("using default sensitive sentences", (done) => {
      const result = AwesomeHelp.checkSensitiveWord(
        "闪电湖手段和速台独度哈师大会实打实大密诱阿萨德化手段时段是的哈USD哈"
      );

      should((result as Map<string, { location: number }[]>).size).equal(2);
      should((result as Map<string, any>).get("台独").length).equal(1);
      done();
    });
    it("support the quick search", (done) => {
      const result = AwesomeHelp.checkSensitiveWord(
        "闪电湖手段和速度哈师大会实打实大密诱阿萨德化手段时段是的哈USD哈",
        true
      );
    ...
    it("support the custom sensitive sentences", (done) => {
      const sensitiveMap = AwesomeHelp.makeSensitiveMap([
        "这是自定义敏感词",
        "我也是",
      ]);
      const result = AwesomeHelp.checkSensitiveWord(
        "闪电湖手段和速度哈师大会实打实大我也是德化手段时段是的这是自定义敏感词哈USD哈输的哈us后端少打我也是啥速度哈大了点啦",
        undefined,
        sensitiveMap
      );

      should((result as Map<string, { location: number }[]>).size).equal(2);
      should((result as Map<string, any>).get("我也是").length).equal(2);
      done();
    });
  });

...
// 隐私号码的设置
export const privacyMobile = (mobile: string) =>
  mobile.replace(/^(\d{3})\d{4}(\d*)$/, '$1****$2');

// 姓名脱敏
export const privacyName = (name: string) =>
  name.replace(/^(\S)(\S|\s)*$/, '$1**');

// 中文以及全角字符
export const chineseAndfullWidthChar =
  /[\u4E00-\u9FA5\uF900-\uFA2D\uFF00-\uFFEF]/;
...
// 匹配整数
export const integer = /^(0|[1-9][0-9]*|-[1-9][0-9]*)$/;
